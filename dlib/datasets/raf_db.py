...
def split_RAFDB(args):
    """
    Create a validation/train sets in RAFDB dataset.
    ...
    origin_cl_int_to_txt = {
        1: constants.SURPRISE,
        2: constants.FEAR,
        3: constants.DISGUST,
        4: constants.HAPPINESS,
        5: constants.SADNESS,
        6: constants.ANGER,
        7: constants.NEUTRAL
    }
    ...
    if crop_align:
        ...
        if align_type == ALIGN_DEEP_FACE:
            ...
            aligner = FaceEvolveAlign(out_size=CROP_SIZE, verbose=False,
                                      no_warnings=True)
    ...
    if use_original_align:
        final_id_tag = "aligned"

    pre_id = 'basic/Image/original/'
    ...
    log = f"{40 * '*'} Log {40 * '*'} \nDataset: {ds}. \n"
    # images + cl.
    with open(join(baseurl, ds, "basic/EmoLabel", "list_patition_label.txt"),
              "r") as fcl:
        ...
        for el in content:
            ...
            idcl, cl = el.split(" ")
            cl = int(cl)
            # switch to the ordered label --------------------------------------
            name_cl_str = origin_cl_int_to_txt[cl]
            cl = ordered_labels[name_cl_str]
            # ------------------------------------------------------------------

            sample_id.append(f"{pre_id}{idcl}")
    ...
    if crop_align and not already_cropped_aligned:
        # bbox + crop faces.
        outfd_crop = join(baseurl, ds, 'basic/Image/cropped')
        os.makedirs(outfd_crop, exist_ok=True)

        outfd_align = join(baseurl, ds, f"basic/Image/{final_id_tag}")
        os.makedirs(outfd_align, exist_ok=True)

        l_files = find_files_pattern(
            join(baseurl, ds, "basic/Annotation/boundingbox"), '*.txt')
        print('Cropping/aligning faces...')
        list_failed_align = []

        for f in tqdm.tqdm(l_files, total=len(l_files), ncols=80):
                ...
                if align:
                    if align_type == ALIGN_DEEP_FACE:
                        ...
                        if (not aligner.success) and (bbox_xyxy is not None):
                            ...
                            aligned_face = np.array(aligned_face).astype(np.uint8)
    ...
    for s in sample_id_cl:
        ...
        if args.use_original_align:
            _s = s.replace('.jpg', '_aligned.jpg')
            pz = join(baseurl, ds, f"basic/Image/{final_id_tag}/{_s}")
            assert os.path.isfile(pz), pz

        new_id = f"basic/Image/{final_id_tag}/{_s}"
...
def split_crop_align_rafdb(root_main: str,
                           crop_align: bool,
                           align: bool,
                           align_type: str,
...
if __name__ == "__main__":
    ...
    already_cropped_aligned = True

    if crop_align and not already_cropped_aligned:
        assert not use_original_align
        resp = input("you want to crop/align faces[time consuming]? y/n?: ")
        if resp == 'y':
            print('Ok. will do.')
        else:
    ...
    split_crop_align_rafdb(root_main=root_dir,
