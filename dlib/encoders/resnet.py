...
class ResNetEncoder(ResNet, EncoderMixin):
    ...
    def wsol_init(self,
        ...
        # wsol: set both to 1 (28x28 outmaps). original: both to 2 (7x7
        # outmaps).
        if large_maps:
            z_stride = 1
            stride_l3 = 1
        ...
        assert isinstance(apply_self_atten, bool), type(apply_self_atten)
        self.apply_self_atten: bool = apply_self_atten

        if atten_layers_id is None:
            assert not apply_self_atten, apply_self_atten
            atten_layers_id = []
        else:
            assert apply_self_atten, apply_self_atten
            assert isinstance(atten_layers_id, list), type(atten_layers_id)
            for z in atten_layers_id:
                assert isinstance(z, int), f"{type(z)} | {z}"

        self.atten_layers_id: list = atten_layers_id
    ...
    def _make_layer(
        ...
        # ADL
        if self.use_adl:
            assert isinstance(split, list), type(split)
    ...
    @staticmethod
    def get_self_attention(features: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, x):
        ...
        n_layers = len(stages)
        if self.apply_self_atten:
            for i in self.atten_layers_id:
                assert i < n_layers, f"{i} | {n_layers}"

        features = []
        for i in range(self._depth + 1):
            x = stages[i](x)

            if self.apply_self_atten and i in self.atten_layers_id:
                att = self.get_self_attention(x)
                x = x * att
