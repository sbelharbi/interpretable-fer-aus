...
__all__ = ['BuiltinCam', 'ReadyCam', 'SegmentationCam']
...
class BuiltinCam:
    """Implements a class activation map extractor over models that construct
     ...
     This is extended to other models that build some attention such as APVIT.
    ...
    def __init__(self,
                 model: Union[STDClassifier, PoolingVitClassifier]
                 ) -> None:

        if not isinstance(model, PoolingVitClassifier):
    ...
    def compute_cams(self, class_idx: int, normalized: bool = True) -> Tensor:
        ...
        if isinstance(self.model, PoolingVitClassifier):
            cams = self.model.cams
            class_idx = 0
            assert not self.support_backgr, self.support_backgr
