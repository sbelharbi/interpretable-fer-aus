...
def build_std_cam_extractor(classifier, args):
    p1 = [constants.GAP,
          constants.ACOL,
          constants.MAXPOOL,
          constants.WILDCATHEAD,
          constants.LSEPOOL,
          constants.PRM
          ]
    mbin1 = [constants.METHOD_WILDCAT,
             constants.METHOD_GAP,
             constants.METHOD_MAXPOOL,
             constants.METHOD_LSE,
             constants.METHOD_ACOL,
             constants.METHOD_PRM
             ]

    mready = [constants.METHOD_TSCAM]
    ...
    # ready cams
    if method in mready:
        assert args.model['arch'] in [constants.TSCAMCLASSIFIER]

        return ReadyCam(model=classifier)

    # builtin
    if method == constants.METHOD_APVIT:
        assert encoder_name == constants.APVIT, f"{encoder_name} | " \
                                                f"{constants.APVIT}"
