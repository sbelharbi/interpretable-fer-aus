...
class Viz_WSOL(object):
    ...
    def tagax_short(self, ax, tag: str):
        ax.text(10, 20,
                tag, weight='bold', color='black', fontsize=20,
                bbox={'facecolor': 'white', 'pad': 1, 'alpha':
                    .99, 'edgecolor': 'white', 'boxstyle': 'round,pad=0.2'}
                )
    ...
    def plot_fer_single(self, datum: dict, outf: str):
        """
        Plot facial expression recognition cam.
        """
        nrows = 1
        ncols = 3
        if datum['heatmap'] is not None:
            ncols += 1
        ...
        fig, axes = plt.subplots(nrows=nrows, ncols=ncols, sharex=False,
                                 sharey=False, squeeze=False, figsize=(fh, fw)
                                 )
        ...
        if 'gt_bbox' in datum.keys():
            ...
            if ncols == 3:
                self._plot_bbox(axes[0, 1],
                                ...
                                pred_bbox=None,
                                cam=datum['cam'],
                                tag=f"CAM-{datum['tag_cl']}")
                self._plot_bbox(axes[0, 2],
                                img=datum['img'],
                                gt_bbox=None,
                                pred_bbox=None,
                                cam=datum['attention'],
                                tag=f"ATT-{datum['tag_cl']}")
                ...
                self._plot_bbox(axes[0, 1],
                                ...
                                pred_bbox=None,
                                cam=datum['heatmap'],
                                tag=f"{datum['tag_heatmap']}-{datum['tag_cl']}")
                self._plot_bbox(axes[0, 2],
                                img=datum['img'],
                                gt_bbox=None,
                                pred_bbox=None,
                ...
                raise NotImplementedError(ncols)
    ...
    def plot_fer_single_less(self,
        ...
        if 'gt_bbox' in datum.keys():
            ...
            self.tagax_short(axes[0, 0], datum['img_class_str'])
            ...
            self.tagax_short(axes[0, i], 'Attention')
        ...
        self.closing(fig, outf, dpi)

    def plot_fer_avg_cam(self, cam: np.ndarray, class_name: str, outf: str):
        plt.close('all')
        ...
        ax.text(20, 30,
                class_name, weight='bold', color='black', fontsize=20,
                bbox={'facecolor': 'white', 'pad': 1, 'alpha':
                    .99, 'edgecolor': 'white', 'boxstyle': 'round,pad=0.5'}
                )
    ...
    def closing(self, fig, outf, dpi: int = None):
        ...
        fig.savefig(outf, pad_inches=0, bbox_inches='tight', dpi=_dpi)
        plt.close(fig)
    ...
    def _watch_plot_perfs_meter(self, meters: dict, split: str, perfs: list,
        ...
        best_epoch = meters[constants.VALIDSET][constants.LOCALIZATION_MTR
        ]['best_epoch']
