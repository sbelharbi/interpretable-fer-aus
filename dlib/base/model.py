...
class STDClModel(torch.nn.Module):
    def __init__(self):
        super(STDClModel, self).__init__()
    ...
    def forward(self, x: torch.Tensor, labels: torch.Tensor = None):
        ...
        self.linear_features = self.classification_head.linear_features
        # self.linear_w = self.classification_head.linear_w

        # segmentation
        if self.segmentation_head is not None:
            self.segmentation_head(features[-1])

        return cl_logits

    @property
    def linear_w(self):
        return self.classification_head.linear_w
