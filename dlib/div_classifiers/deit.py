...
class TSCAM(VisionTransformer):
    def __init__(self, encoder_name: str, encoder_weights: str, *args,
        ...
        # todo: improve the classification head.
    ...
    def forward(self, x, labels=None):
        ...
        x_patch = x_patch.contiguous()  # b, embed_dim, 14, 14.
        x_cams = self.head(x_patch)
        x_logits = self.avgpool(x_cams).squeeze(3).squeeze(2)


        attn_weights = torch.stack(attn_weights)        # 12 * B * H * N * N
        attn_weights = torch.mean(attn_weights, dim=2)  # 12 * B * N * N

        feature_map = x_cams.detach().clone()    # B * C * 14 * 14
        ...
        semantic_agnostic_att_map = attn_weights.sum(0)[:, 0, 1:].reshape([n, h, w]).unsqueeze(1)
        # b, 1, 14, 14.

        self.features = [x,
                         x_patch,
                         semantic_agnostic_att_map]
        # features at index 0 are not used by convention in other arch such as
        # resnet. by default, it contains the input image.
        self.att_maps = semantic_agnostic_att_map

        if labels is not None:
            cams = semantic_agnostic_att_map * feature_map  # B * C * 14 * 14
